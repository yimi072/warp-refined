...
pub enum LaunchMode {
    ...
    /// Remote server proxy 鈥?bridges SSH stdio to the daemon's Unix socket.
    /// This is a short-lived process that runs for the lifetime of an SSH session.
    RemoteServerProxy,

    /// Remote server daemon 鈥?long-lived headless process serving remote
    ...
    if let Some(command) = args.command() {
        ...
        match command {
            ...
            warp_cli::Command::Worker(warp_cli::WorkerCommand::RemoteServerProxy(args)) => {
                // Proxy is a thin byte bridge (stdin/stdout 鈫?Unix socket).
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    // 鈹€鈹€ Early initialization (pre-AppBuilder) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
pub(crate) fn initialize_app(
    ...
    if user_is_logged_in {
        // Skip refresh_user for CLI mode 鈥?the CLI handles auth refresh in
    ...
    });
    {
        ...
        // the org level). Billing metadata 鈥?including `warp_ai_policy.is_voice_enabled`
        // 鈥?lives inside the team data, so `TeamsChanged` covers all policy updates.
...
fn launch(ctx: &mut warpui::AppContext, app_state: Option<AppState>, launch_mode: LaunchMode) {
    ...
    match launch_mode {
        ...
        // Proxy should never reach launch() 鈥?it's a thin byte bridge.
        LaunchMode::RemoteServerProxy => {
